...
import { useUserBookings, DbBooking } from '@/hooks/useBookings';
import { usePropertyById } from '@/hooks/useProperties';

function BookingCard({ booking, isPast = false }: { booking: DbBooking; isPast?: boolean }) {
  const { data: property, isLoading } = usePropertyById(booking.property_id);
...
export default function Bookings() {
  ...
  const upcomingBookings = bookings.filter(b => !isPast(parseISO(b.check_in)));
  const pastBookings = bookings.filter(b => isPast(parseISO(b.check_in)));
